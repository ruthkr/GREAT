#' Flag data to transform time points which overlap to data target timecourse
#'
#' `get_compared_timepoints` flags data to transform time points which overlap to data target timecourse by comparing each data to transform time points to minimum and maximum value of data target.
#'
#' @param data Input data containing both data to transform and data target.
...
get_compared_timepoints <- function(data,
  ...
  data$is_compared[(data$accession == accession_data_to_transform & (data$shifted_time >= min_data_target & data$shifted_time <= max_data_target))] <- TRUE

  # Get the extreme data target times which used - bigger or equal than max of data to transform, and smaller or equal than  min data to transform, because have to project data to transform onto data target
  ...
  max_data_target <- max_is_compared_to_data_to_transform(max_data_to_transform, data[data$accession == accession_data_target, ])
  min_data_target <- min_is_compared_to_data_to_transform(min_data_to_transform, data[data$accession == accession_data_target, ])
...
interpolate_data_target_comparison_expression <- function(data_to_transform_time,
                                                          data_target_dt) {

  data_target_dt$diff <- data_target_dt$shifted_time - data_to_transform_time
  ...
  # Otherwise, cut down data_target observations to the two nearest timepoints to the data_to_transform time
  data_target_dt$diff <- abs(data_target_dt$shifted_time - data_to_transform_time)
