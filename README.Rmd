...
`greatR` (**G**ene **R**egistration from **E**xpression **a**nd **T**ime-courses in **R**) is a tool to register (align) two sets of gene expression profiles that users wish to compare. These gene profiles data will be referred as the query and the reference data. To match the ranges over time between those profiles, the timepoints of query gene expression profiles will be transformed through stretching and shifting process. This tool uses a Bayesian approach based on statistical model comparison to evaluate the optimality of gene expression profiles alignment.
...
3. [Visualise registration results](https://ruthkr.github.io/greatR/articles/visualise-results.html)
