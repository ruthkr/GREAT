...
alex_data_all <- system.file("extdata/brapa_arabidopsis_all_replicates.csv", package = "GREAT") %>%
  utils::read.csv()

alex_data_mean <- system.file("extdata/brapa_arabidopsis_mean.csv", package = "GREAT") %>%
  utils::read.csv()
...
alex_data_all %>% 
  ...
  ggplot() +
  aes(x = timepoint, y = expression_value, colour = group_simplified) +
  geom_point() +
...
ggplot() +
  aes(x = timepoint, y = expression_value, color = accession) +
...
```{r plot-original-data, fig.width=10, fig.height=10, warning=FALSE}
GREAT::plot_registered_gene_of_interest(test_using_brapa_data[['imputed_mean_df']])
```
...
ap1_data <- data_to_plot %>% 
  dplyr::filter(locus_name == "AP1") %>% 
  dplyr::as_tibble()

ap1_data %>% 
  dplyr::select(bra_gene, shifted_time, expression_value,accession, locus_name) %>% 
  # dplyr::pull(bra_gene) %>% 
  # unique()
  GREAT::plot_registered_gene_of_interest()
```
