...
```{r fn-get-poiss-vect}
get_poiss_vect <- function(table, sd, num) {
  poiss_vec <- table %>%
    dplyr::filter(
      SD == sd,
      num_timepoints == num
    ) %>%
    dplyr::pull(registered_genes_total10)
...
```{r fn-do-t-test}
do_t_test <- function(table, sd, num_a, num_b) {
  t_test_results <- t.test(
    get_poiss_vect(table, sd, num_a),
    get_poiss_vect(table, sd, num_b)
  )
...
tidyr::crossing(
  ...
  dplyr::select(SD, num_timepoints_a = V1, num_timepoints_b = V2) %>%
  dplyr::arrange(SD, num_timepoints_a, num_timepoints_b) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(
    SD = SD %>% as.character() %>% as.numeric(),
    t_test_p_value = do_t_test(table, SD, num_timepoints_a, num_timepoints_b)$p.value
  ) %>%
  ggplot() +
  aes(x = SD, y = t_test_p_value, color = interaction(num_timepoints_a, num_timepoints_b)) +
