...
undersample_data_timepoints_skip <- function(data, keep_first_last_timepoint = TRUE, skip_timepoints_pattern = c(TRUE)) {
  ...
  sliced_data <- data %>%
    dplyr::group_by(locus_name, accession, group) %>%
...
```{r}
reg_with_full_simulated_data$model_comparison_dt %>% dplyr::filter(BIC_registered_is_better == TRUE) %>% nrow()
```

```{r running-multiple-simulation, warning=FALSE, include=FALSE}
# replicate_sd_list <- seq(0, 2, length.out = 21)
replicate_sd_list <- c(0, 0.5, 1)
num_timepoints <- 10
...
for (rep_sd in replicate_sd_list) {
  ...
  skip_1_timepoint_all <- undersample_data_timepoints_skip(simulated_data, keep_first_last_timepoint = TRUE, skip_timepoints_pattern = c(FALSE, TRUE))
  ...
  skip_2_timepoint_all <- undersample_data_timepoints_skip(simulated_data, keep_first_last_timepoint = TRUE, skip_timepoints_pattern = c(FALSE, FALSE, TRUE))
  ...
  results_list[[paste0("data_sd_", stringr::str_pad(rep_sd, width = 2, pad = "0"))]] <- list(
    ...
    all_data_skip1 = skip_1_timepoint_all,
    mean_data_skip1 = skip_1_timepoint_mean,
    all_data_skip2 = skip_2_timepoint_all, 
    mean_data_skip2 = skip_2_timepoint_mean, 
    sd = rep_sd
  )
  
  # results_list[[paste0("mean_data_sd_", stringr::str_pad(rep_sd, width = 2, pad = "0"))]] <- simulated_data_mean
}
...
reg_with_full_simulated_data_skip1 <- results_list %>%
  purrr::map(
    function(x) {
      results <- GREAT::scale_and_register_data(
        x$mean_data_skip1 %>% dplyr::mutate(tissue = "apex"),
        x$all_data_skip1 %>% dplyr::mutate(tissue = "apex"),
        stretches =  c(4, 3, 2, 1),
        shift_extreme = 5,
        num_shifts = 21,
...
reg_with_full_simulated_data_skip2 <- results_list %>%
  purrr::map(
    function(x) {
      results <- GREAT::scale_and_register_data(
        x$mean_data_skip2 %>% dplyr::mutate(tissue = "apex"),
        x$all_data_skip2 %>% dplyr::mutate(tissue = "apex"),
        stretches =  c(4, 3, 2, 1),
        shift_extreme = 5,
        num_shifts = 21,
...
reg_with_full_simulated_data %>% 
  purrr::map(
    ~ data.frame(
      sd = .x$sd,
      registered_genes = .x$model_comparison_dt %>% 
...
reg_with_full_simulated_data_skip1 %>% 
  purrr::map(
    ~ data.frame(
      sd = .x$sd,
      registered_genes = .x$model_comparison_dt %>% 
...
reg_with_full_simulated_data_skip2 %>% 
  purrr::map(
    ~ data.frame(
      sd = .x$sd,
      registered_genes = .x$model_comparison_dt %>% 
...
```{r}
results_list$data_sd_00$mean_data_skip1
```
...
results_list$data_sd_00$all_data_skip1 %>%
  ggplot() +
  aes(x = timepoint, y = mean_cpm, colour = accession) +
  geom_point() +
  geom_line() +
  facet_wrap(group ~ locus_name)


results_list$data_sd_00$all_data
```
