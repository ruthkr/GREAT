...
``` r
do.initial_rescale <- 'rescale' # should be 'rescale' if want to use scaled df for registration, rather than mean_df
...
O <- prepare_scaled_and_registered_data(mean_df, all.data.df,
  stretch = stretch, initial_rescale, should.rescale, min_num_overlapping_points,
...
| locus\_name      | accession | tissue | timepoint |   mean_cpm | stretched.time.delta | shifted_time | is.registered |
|:-----------------|:----------|:-------|----------:|-----------:|---------------------:|-------------:|:--------------|
