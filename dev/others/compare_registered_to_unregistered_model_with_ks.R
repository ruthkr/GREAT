compare_registered_to_unregistered_model_with_ks <- function(curr.sym, all.data.df, is.testing = FALSE, accessions = c("Col0", "Ro18"), n_input = 0, shape = c(0, 0), new_shape = FALSE) {
  ...
  if (new_shape) {
    ara.spline.data <- ara.spline.data %>%
      dplyr::mutate(
        shifted.time = (shifted.time * shape[[1]]) + shape[[2]]
      )
  }
  ...
  if (n_input > 0) {
    ara.spline.data_comb <- ara.spline.data %>%
      ...
      expand_y_space(shifted.time, mean.cpm, n = n_input) %>%
      distinct()
  } else {
    ara.spline.data_comb <- ara.spline.data
  }
  ...
  combined.spline.data <- as.data.table(
    rbind(
      ara.spline.data_comb %>% dplyr::select(shifted.time, mean.cpm),
      bra.spline.data %>% dplyr::select(shifted.time, mean.cpm)
    )
  )
  ...
  if (is.testing == TRUE) {
    ...
    spline.df <- rbind(
      ara.pred.df %>% dplyr::select(shifted.time, mean.cpm),
      bra.pred.df %>% dplyr::select(shifted.time, mean.cpm),
      combined.pred.df %>% dplyr::select(shifted.time, mean.cpm)
    )
