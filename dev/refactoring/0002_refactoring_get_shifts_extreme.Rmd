...
```{r}
best_shifts$data_align_compared_mean[best_shifts$gene == "BRAA02G018970.3C"] %>% 
  length()

# Get list of gene
best_shifts$gene %>% unique()

# Get the compared mean from data_tp_align (Arabidopsis), remember that there is only one copy in Ara, thats why when compared, there are some duplicated 
best_shifts$data_align_compared_mean
# Take the unique ---------
best_shifts$data_align_compared_mean %>% unique()
```
