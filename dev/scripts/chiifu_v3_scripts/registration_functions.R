...
scale_all_rep_data <- function(mean_df, all.rep.data, scale.func) {
  ...
  # ggplot(mean_df[mean_df$locus_name=='BRAA01G000040.3C', ], aes(x=timepoint, y=mean_cpm, color=accession)+
  #          geom_point()
...
filter.low.variability <- function(exp, c.th) {
  ...
  exp[, C:=cor(mean_cpm, norm.cpm, method='pearson'), by=.(locus_name, accession, tissue)]
...
load_mean_df <- function() {
  ...
  sumBrassicas <- F # if false use separate brassica genes, and compare to repeated arabidopsis genes. If true, sume copies of each brassica and compare to arabidopsis
...
prepare_scaled_and_registered_data <- function(mean_df, all.data.df, stretches,
  ...
  if (initial.rescale==TRUE) {
    ...
    # ggplot(all.data.df[all.data.df$locus_name=='BRAA01G000040.3C'], aes(x=timepoint, y=mean_cpm, color=accession))+
    #   geom_point()
  } else {
    to.shift.df <- copy(mean_df)
  }
  # ggplot(to.shift.df[to.shift.df$locus_name=='BRAA03G004600.3C'], aes(x=timepoint, y=mean_cpm, color=accession))+
  #   geom_point()
  # tst <- all.data.df
  # ggplot(tst[tst$locus_name=='BRAA01G000040.3C'], aes(x=timepoint, y=mean_cpm, color=accession))+
  #   geom_point()
  ...
  #        aes(x=shifted_time, y=mean_cpm, color=accession))+
  #   geom_point()+
  #   geom_line()
...
get_best_result <- function(df) {
  ...
  if(sum(is_best)==1) {
    ...
    cand.stretches <- df$stretch[is_best]
    ...
    is_best[df$stretch != min.stretch] <- FALSE
    if(sum(is_best)==1) {
      return(is_best)
    } else {
      cand.shifts <- df$shift[is_best]
...
get_best_stretch_and_shift <- function(to.shift.df, all.data.df, stretches, do_rescale, min_num_overlapping_points, shift_extreme) {
  ...
  # for the best shift in each stretch, compares to separate models to calculate AIC/BIC under registration,
  # / no registration.
  ...
  # - model_comparison.dt : AIC / BIC scores for best registerd model found, compared to separate model for
  ...
  for (i in 1:length(stretches)) {
    ...
    # ggplot(to.shift.df[to.shift.df$locus_name=='MSTRG.12467',], aes(x=timepoint, y=mean_cpm, color=accession))+
    #   geom_point()
    ...
    # gene's expression separately in Col0 and Ro18
  ...
  all_model_comparison.dt <- do.call('rbind', all_model_comparison.dt) # model comparison of best shift (for each stretch) to separate modeles
  ...
  # how much better register.BIC is than separate.BIC
  all_model_comparison.dt$delta.BIC <- all_model_comparison.dt$registered.BIC - all_model_comparison.dt$separate.BIC
  all_model_comparison.dt[, is_best:=(delta.BIC==min(delta.BIC)), by=.(gene)] # best is one for which registered.BIC is as small as possible compared to separate.BIC
...
calculate_all_model_comparison_stats <- function(all.data.df, best_shifts) {
  ...
  # tst <- all.data.df
  # ggplot(tst[tst$locus_name=='BRAA01G000040.3C'], aes(x=shifted_time, y=mean_cpm, color=accession))+
  #   geom_point()
  # ggplot(tst[tst$locus_name=='BRAA01G000040.3C'], aes(x=timepoint, y=mean_cpm, color=accession))+
  #   geom_point()
  ...
  out <- data.table(data.frame('gene'=genes, 'separate.AIC'=out.sepAIC, 'registered.AIC'=out.combAIC,
                               'separate.BIC'=out.sepBIC, 'registered.BIC'=out.combBIC))
  return(out)
}
...
compare_registered_to_unregistered_model <- function(curr.sym, all.data.df, is.testing) {
  ...
  # ggplot(curr.data.df, aes(x=shifted_time, y=mean_cpm, shape=is.compared, color=accession))+
  #   geom_point()
  ...
  ara.fit <- lm(mean_cpm~splines::bs(shifted_time, df=num.spline.params, degree=3), data=ara.spline.data)
  bra.fit <- lm(mean_cpm~splines::bs(shifted_time, df=num.spline.params, degree=3), data=bra.spline.data)
  combined.fit <- lm(mean_cpm~splines::bs(shifted_time, df=num.spline.params, degree=3), data=combined.spline.data)
  ...
  if (is.testing==TRUE) {
    ...
    ggplot(data=combined.spline.data, aes(x=shifted_time, y=mean_cpm,
                                          colour=accession))+
      geom_point()+
      geom_line(data=spline.df)+
      ggtitle(paste0(curr.sym, ' : sep AIC:combo AIC=', round(separate.AIC), ':', round(combined.AIC),
  ...
  return(list(separate.AIC, combined.AIC, separate.BIC,combined.BIC))
}
...
impute_arabidopsis_values <- function(shifted.mean_df) {
  ...
  # ggplot(shifted.mean_df[shifted.mean_df$locus_name=='BRAA01G001090.3C', ], aes(x=shifted_time, y=mean_cpm, color=accession))+
  #   geom_point()
  # # an unregistered one
  # ggplot(shifted.mean_df[shifted.mean_df$locus_name=='BRAA01G003140.3C', ], aes(x=shifted_time, y=mean_cpm, color=accession))+
  #   geom_point()
  ...
  for (curr.gene in unique(shifted.mean_df$locus_name)) {
    ...
    # ggplot(curr.df, aes(x=shifted_time, y=mean_cpm, color=accession))+
    #   geom_point()+
...
calculate_all_best_shifts <- function(mean_df, stretch_factor, do_rescale, min_num_overlapping_points, shift_extreme) {
  ...
  #for (curr_sym in unique(mean_df$locus_name)) {
    ...
    # ggplot(mean_df[mean_df$locus_name==curr_sym,], aes(x=timepoint, y=mean_cpm, color=accession))+
    #   geom_point()
...
get_best_shift <- function(curr_sym, test, stretch_factor, do_rescale, min_shift, max_shift, testing=FALSE) {
  ...
  for (i in 1:length(all_shifts)) {
    ...
    # if (testing==TRUE) {
    #   p <- ggplot(test, aes(x=shifted_time, y=mean_cpm, color=accession))+
    #     geom_point()+
    #     ggtitle(paste0('shift : ', curr_shift))
    #   p
    #   ggsave(paste0('./testing/', curr_shift, '.pdf'))
    # }
    ...
    if (do_rescale==TRUE) {
      ...
      ara.sd <- sd(compared$mean_cpm[compared$accession=='Col0'])
      bra.sd<- sd(compared$mean_cpm[compared$accession=='Ro18'])
    ...
    if (testing==TRUE) {
      p <- ggplot(compared, aes(x=shifted_time, y=mean_cpm, color=accession))+
        geom_point()+
        ggtitle(paste0('shift : ', curr_shift))
        ggsave(paste0('./testing/',stretch_factor, '-', curr_shift, '.pdf'))
    }
...
apply_best_shift <- function(mean_df, best_shifts) {
  ...
  for (curr.gene in unique(test$locus_name)) {
    ...
    # ggplot(tmp, aes(x=shifted_time, y=mean_cpm, color=accession))+
    #    geom_point()
  }
