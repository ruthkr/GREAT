...
```{r all-data-illustration, echo=FALSE, out.width='80%'}
...
`GREAT` package provides a helper function `get_expression_of_interest()` to combine reference and query (data to register) data frames, if table ID which connect reference and query data frames provided (as illustrated in the figure below). 

<br>

```{r combine-data, echo=FALSE, out.width='120%'}
knitr::include_graphics("figures/Prepare_data_plus.png")
```
