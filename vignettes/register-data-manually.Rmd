...
```{r register-subset-data, message=FALSE, warning=FALSE, eval=FALSE}
registration_results <- register(
  b_rapa_data,
  reference = "Ro18",
  query = "Col0",
...
#> 
#> ── Starting manual registration ────────────────────────────────────────────────
#> ✔ Applying registration for genes (10/10) [42.9s]
```

The same as previous registration process, users can analyse further the registration result by following [visualising results](https://ruthkr.github.io/greatR/articles/visualise-results.html) vignette.
