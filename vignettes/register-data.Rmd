...
To align gene expression time-course between *Arabidopsis* Col-0 and *B. rapa* Ro18, we can use function `register()`. By default, the best registration parameters are optimised via Simulated Annealing. When using the default `optimise_registration_parameters = TRUE`, the stretch and shift search space is automatically estimated.
...
## Getting the registration results

Function `register()` returns a list of two frames:

- `data` is a data frame containing the scaled expression data and an additional \code{timepoint_reg} column which is a result of registered time points by applying registration parameters to the query data.
- `model_comparison` is a data frame containing both the Bayesian Information Criterion (BIC) for the separate model (`BIC_separate`) and for the combined model (`BIC_combined`) after applying optimal registration parameters for each gene. If the value of `BIC_separate` > `BIC_combined`, then expression dynamics between reference and query data can be registered (`registered = TRUE`).
