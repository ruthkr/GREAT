...
The function `summary()` returns a list containing three different objects:
...
The function `plot()` allows users to plot the registration results of the genes of interest.
...
- `result` distance between scaled reference and query expressions using registered time points.
...
Each of these data frames above can be visualised using the `plot()` function, by selecting either `type = "result"` or `type = "original"`.
